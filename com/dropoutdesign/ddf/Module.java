...
public class Module {
	...
	if (currentConnection != null) {
	    ...
	    if (lastFailureTime == 0 || (int)(System.currentTimeMillis() - lastFailureTime) >= RETRY_INTERVAL_MS) {
		try {
		    currentConnection = ModuleConnection.open(address);
		    System.out.println(currentConnection.getName()+" firmware: "+Integer.toString(currentConnection.firmwareVersion(), 16));
		    System.out.println(currentConnection.getName()+" i2c: "+Integer.toString(currentConnection.checkI2C(), 16));
		    currentConnection.reset(); // send soft reset command to module
		    //System.out.println("Response to self-test: "+Integer.toString(currentConnection.selfTest(), 16));
		}
	...
	closeConnection(); 
	lastFailureTime = System.currentTimeMillis();
        System.out.println("Error occurred with module at "+address+". currentConnection="+currentConnection+". lastFailureTime="+lastFailureTime+".");
    }
}
